...
// これにより、既存のReact環境と衝突せずに地図機能を利用できます
...
/**
 * WorldDashboard v2.7 - Organic Glass UI Edition
 * * 安定化アップデート:
 * 1. 地図ライブラリのインポートを修正（React競合の解消）
 * 2. 有機的な硝子（Glassmorphism）デザインの徹底適用
 * 3. 各国詳細パネルの全指標（FSI, 成長率等）をHUDスタイルで復活
 * 4. リアルタイムニュースフィード（Google Alerts RSS対応）の統合
 * 5. 地図の移動制限の最適化（上下は地図の端でロック、左右は自由）
 */
    ...
    <div className="w-full h-full bg-slate-950">
      <ComposableMap projectionConfig={{ scale: 220 }} className="w-full h-full">
        <ZoomableGroup 
          ...
          /* 左右は自由、上下は地図サイズ(0-600)に厳密にロックし、虚無が見えないようにします */
          ...
          <div className="flex-1 min-h-0">
            <ResponsiveContainer>
              <PieChart>
                ...
                <ChartTooltip contentStyle={{ backgroundColor: 'rgba(2, 6, 23, 0.95)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '1.5rem', color: '#fff', fontSize: 11 }} itemStyle={{ color: '#fff' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="lg:col-span-4 bg-slate-950/40 backdrop-blur-[60px] border border-white/10 flex flex-col overflow-hidden rounded-[3.5rem] shadow-2xl animate-in slide-in-from-right duration-700">
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
            {news.map((item, i) => (
              <a key={i} href={item.link} target="_blank" rel="noreferrer" className="block p-5 bg-white/[0.04] hover:bg-white/[0.08] border border-transparent hover:border-cyan-500/20 rounded-[2rem] transition-all group active:scale-[0.98] duration-300">
                <div className="text-[10px] text-slate-500 mb-3 flex justify-between font-mono">
                  <span className="bg-cyan-500/10 text-cyan-400 px-3 py-0.5 rounded-full font-bold">{new Date(item.pubDate).toLocaleDateString()}</span>
    ...
    <div className="flex flex-col h-full bg-slate-900/30 backdrop-blur-[80px] border-l border-white/10 shadow-[-30px_0_60px_rgba(0,0,0,0.5)] overflow-hidden">
      <div className="p-10 border-b border-white/5 flex justify-between items-start bg-gradient-to-b from-white/[0.04] to-transparent">
        <div className="space-y-3">
          ...
          <div className="flex gap-3 mt-3 font-mono">
             ...
             <span className="bg-white/5 text-slate-400 text-[10px] px-3 py-1 rounded-full border border-white/10 uppercase font-black tracking-widest">{canonical?.politics?.regime_type || 'N/A'}</span>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-10 scrollbar-thin">
        ...
        <div className="flex flex-wrap gap-3 pt-4">
           {ui_view?.tags?.map(t => (
             <span key={t} className="px-4 py-1.5 rounded-full border border-white/10 text-[10px] text-slate-400 uppercase font-black bg-white/[0.05] hover:bg-cyan-500/10 transition-all cursor-pointer shadow-lg tracking-widest">
               #{t}
             </span>
           ))}
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    // 確実に正しいパスでデータを取得（GitHub Pages対応）
    ...
    fetch(`${baseUrl}worlddash_global_master.json`)
      ...
      .then(setData)
      .catch(e => console.error("Neural data link failed", e));
  }, []);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 relative font-sans text-slate-200">
      ...
      {/* Header */}
      <header className="absolute top-0 left-0 right-0 h-28 flex items-center px-16 justify-between z-[80] bg-gradient-to-b from-slate-950/90 to-transparent pointer-events-none">
        <div className="flex items-center gap-6 pointer-events-auto">
          <div className="p-4 bg-cyan-500/10 rounded-[2rem] border border-cyan-500/20 backdrop-blur-3xl shadow-[0_0_40px_rgba(6,182,212,0.1)]">
            <Globe className="text-cyan-400 animate-pulse" size={32} />
          </div>
          <div>
            <h1 className="text-3xl font-black tracking-[0.5em] font-mono text-white text-shadow-glow flex items-center gap-2 uppercase">WORLD<span className="text-cyan-400 opacity-90">DASH</span></h1>
            <div className="text-[10px] text-slate-500 font-black uppercase tracking-[0.6em] mt-1 tracking-widest opacity-60">Intelligence_Nexus_v2.7</div>
          </div>
        </div>
      ...
      <main className="flex-1 relative">
        ...
        {hoverInfo && (
          <div className="fixed z-[120] px-8 py-5 bg-slate-900/80 backdrop-blur-[40px] border border-white/10 text-slate-100 font-mono pointer-events-none shadow-2xl rounded-[2.5rem] animate-in fade-in zoom-in-95 duration-300" style={{ left: hoverInfo.x + 30, top: hoverInfo.y + 30 }}>
            ...
            <div className="opacity-40 text-[11px] tracking-[0.5em] font-black uppercase flex justify-between gap-16 font-mono"><span>REF_NODE</span><span className="text-white">{hoverInfo.iso3}</span></div>
          </div>
        )}

        <aside className={`absolute top-0 bottom-0 right-0 w-[28rem] md:w-[36rem] transform transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) z-[90] ${selectedIso ? 'translate-x-0' : 'translate-x-full'}`}>
          <CountryDetails country={countryByIso[selectedIso]} onClose={() => setSelectedIso(null)} />
        </aside>

        <footer className={`absolute bottom-0 left-0 right-0 z-[100] bg-slate-950/60 backdrop-blur-[70px] border-t border-white/10 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col ${isAnalyticsOpen ? 'h-[calc(100vh-6.5rem)] rounded-t-[6rem]' : 'h-16'} shadow-[0_-30px_100px_rgba(0,0,0,0.8)] overflow-hidden`}>
          <button onClick={() => setIsAnalyticsOpen(!isAnalyticsOpen)} className="h-16 w-full flex items-center justify-center gap-8 text-[11px] font-black tracking-[1.2em] text-cyan-400/40 hover:text-cyan-400 transition-all shrink-0 group pointer-events-auto border-b border-white/5">
            <Activity size={20} className={`${isAnalyticsOpen ? 'animate-pulse text-cyan-400' : 'opacity-30 group-hover:opacity-100'}`} /> 
            {isAnalyticsOpen ? 'DEACTIVATE_HUB' : 'ACCESS_GLOBAL_DATA_STREAM'} 
            {isAnalyticsOpen ? <ChevronDown size={28} className="mt-1" /> : <ChevronUp size={28} className="mb-1" />}
          </button>
          <div className="flex-1 overflow-hidden p-12 md:p-24">
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.05); border-radius: 30px; }
