...
const BGM_URL = "https://cdn.pixabay.com/audio/2022/03/10/audio_c330f8a156.mp3"; 
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/30 backdrop-blur-[50px] border-l border-white/10 shadow-[-30px_0_60px_rgba(0,0,0,0.5)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-10 space-y-10 scrollbar-thin">
        <div className="p-8 rounded-[2.5rem] bg-cyan-500/[0.03] border border-cyan-500/10 font-bold text-sm text-slate-200 leading-relaxed min-h-[5.5rem] shadow-inner relative overflow-hidden group">
        ...
        <div className="h-72 border border-white/5 rounded-[3rem] bg-white/[0.02] p-8 shadow-2xl relative group">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="85%" data={radarData}>
              ...
              <Radar name="Status" dataKey="score" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.25} dot={{ r: 4, fill: '#22d3ee' }} />
            </RadarChart>
          </ResponsiveContainer>
        </div>

        <div className="flex flex-wrap gap-3 pt-4">
           {ui_view?.tags?.map(t => <span key={t} className="px-4 py-1.5 rounded-full border border-white/10 text-[10px] text-slate-400 uppercase font-black bg-white/[0.05] hover:bg-cyan-500/10 transition-all cursor-pointer shadow-lg">#{t}</span>)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 relative font-sans text-slate-200">
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          <button onClick={toggleAudio} className={`p-4 rounded-full backdrop-blur-3xl border transition-all duration-500 group shadow-2xl relative ${isAudioActive ? 'bg-cyan-500/20 border-cyan-500/40 text-cyan-400' : 'bg-white/[0.03] border-white/5 text-slate-500'}`}>
            ...
            <div className="absolute top-full mt-4 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-900/90 border border-white/10 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.2em] whitespace-nowrap shadow-2xl">
              {isAudioActive ? 'Neural_Audio: Online' : 'Neural_Audio: Standby'}
            </div>
          </button>
      ...
      <main className="flex-1 relative">
        ...
        <footer className={`absolute bottom-0 left-0 right-0 z-[100] bg-slate-950/60 backdrop-blur-[60px] border-t border-white/10 transition-all duration-1000 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col ${isAnalyticsOpen ? 'h-[calc(100vh-6rem)] rounded-t-[5rem]' : 'h-14'} shadow-[0_-30px_80px_rgba(0,0,0,0.7)] overflow-hidden`}>
          <button onClick={() => setIsAnalyticsOpen(!isAnalyticsOpen)} className="h-14 w-full flex items-center justify-center gap-6 text-[10px] font-black tracking-[1em] text-cyan-400/40 hover:text-cyan-400 transition-all shrink-0 group pointer-events-auto border-b border-white/5">
            <Activity size={18} className={`${isAnalyticsOpen ? 'animate-pulse text-cyan-400' : 'opacity-30 group-hover:opacity-100'}`} /> 
            {isAnalyticsOpen ? 'CLOSE_SYSTEM_HUB' : 'OPEN_INTELLIGENCE_STREAM'} 
