...
// ニュースフィードのURL（Googleアラート等のRSSをここに設定可能）
    ...
    <div className="w-full h-full bg-slate-950">
      <ComposableMap projectionConfig={{ scale: 220 }} className="w-full h-full">
        <ZoomableGroup 
          ...
          minZoom={1} 
          maxZoom={8} 
          // 修正: 左右・上下の移動制限を緩和。アラスカからロシア東端まで移動可能に
...
const GlobalAnalytics = ({ data, isExpanded }) => {
  ...
  return (
    <div className={`grid gap-6 h-full transition-all ${isExpanded ? 'lg:grid-cols-12' : 'lg:grid-cols-2'}`}>
      <div className={`${isExpanded ? 'lg:col-span-8' : ''} grid md:grid-cols-2 gap-4 h-full`}>
        ...
        <div className="bg-slate-900/60 backdrop-blur-md p-4 border border-primary/20 flex flex-col rounded-lg">
          <h4 className="text-[10px] text-primary font-bold uppercase tracking-widest mb-4 flex items-center gap-2">
             <Activity size={12}/> ECONOMIC DISTRIBUTION
          </h4>
          <div className="flex-1">
            <ResponsiveContainer>
              <PieChart>
                ...
                <ChartTooltip 
                  contentStyle={{ backgroundColor: '#020617', borderColor: '#334155', color: '#fff', fontSize: 11 }}
                  itemStyle={{ color: '#fff' }}
                />
              </PieChart>
            </ResponsiveContainer>
      ...
      {/* ニュースフィードセクション */}
      {isExpanded && (
        <div className="lg:col-span-4 bg-slate-900/80 backdrop-blur-xl border border-primary/30 flex flex-col overflow-hidden rounded-lg shadow-[0_0_30px_rgba(6,182,212,0.1)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
            {news.map((item, i) => (
              <a key={i} href={item.link} target="_blank" rel="noreferrer" className="block p-3 bg-slate-900/60 border border-white/5 hover:border-primary/40 rounded transition-all group">
                <div className="text-[9px] text-slate-500 mb-1 flex justify-between font-mono">
            ...
            {news.length === 0 && !loading && (
              <div className="h-full flex flex-col items-center justify-center text-slate-600 font-mono text-[10px] gap-2">
                <AlertCircle size={20}/> NO INTEL RECEIVED
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900/90 backdrop-blur-2xl border-l border-primary/30 shadow-2xl">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin">
        <div className="p-4 rounded bg-primary/5 border-l-2 border-primary font-mono text-xs text-slate-300 leading-relaxed min-h-[5.5rem] shadow-inner">
          {headline}<span className="animate-pulse text-primary font-bold">|</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 bg-slate-800/60 border border-white/5 rounded-md">
            <div className="text-[9px] text-slate-500 mb-1 flex items-center gap-1"><Users size={10}/>POPULATION</div>
            <div className="font-mono text-emerald-400 text-lg">{canonical?.society?.population?.value?.toLocaleString() || '0'}</div>
          </div>
          <div className="p-3 bg-slate-800/60 border border-white/5 rounded-md">
            <div className="text-[9px] text-slate-500 mb-1 flex items-center gap-1"><Activity size={10}/>GDP (NOM)</div>
            <div className="font-mono text-blue-400 text-lg">${((canonical?.economy?.gdp_nominal?.value || 0) / 1e9).toFixed(1)}B</div>
          </div>
        </div>
        <div className="h-52 border border-white/10 rounded-lg bg-slate-950/40 p-3 shadow-xl">
        ...
        <div className="flex flex-wrap gap-2 pt-2">
           {ui_view?.tags?.map(t => <span key={t} className="px-2 py-0.5 rounded border border-white/10 text-[9px] text-slate-400 uppercase font-mono bg-white/5">{t}</span>)}
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  if (!data) return (
    <div className="h-screen flex flex-col items-center justify-center text-primary animate-pulse font-mono bg-slate-950 tracking-[0.4em]">
       <Globe size={40} className="mb-6 animate-spin-slow text-primary/50" />
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 relative font-sans text-slate-200">
      ...
      <header className="absolute top-0 left-0 right-0 h-14 flex items-center px-6 justify-between z-[80] bg-gradient-to-b from-slate-950/80 to-transparent pointer-events-none">
        <div className="flex items-center gap-3 pointer-events-auto">
          ...
          <h1 className="text-lg font-bold tracking-widest font-mono text-glow">WORLD<span className="text-primary">DASH</span></h1>
        </div>
        <button onClick={toggleFs} className="pointer-events-auto text-slate-400 hover:text-primary transition-all flex items-center gap-2 border border-white/10 px-3 py-1.5 rounded bg-slate-900/60 backdrop-blur-md text-[10px] font-bold shadow-lg">
          {isFullscreen ? <Minimize size={14} /> : <Maximize size={14} />} {isFullscreen ? 'EXIT_FULL' : 'ENTER_FULL'}
        </button>
      </header>
      ...
      <main className="flex-1 relative">
        ...
        {/* 地図ホバー・ツールチップ */}
        {hoverInfo && (
          <div className="fixed z-[120] px-3 py-2 text-[10px] bg-slate-900/95 backdrop-blur-xl border border-primary/50 text-slate-100 font-mono pointer-events-none shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-opacity duration-200" style={{ left: hoverInfo.x + 15, top: hoverInfo.y + 15 }}>
        ...
        {/* 右サイドHUD (選択時スライドイン) */}
        <aside className={`absolute top-0 bottom-0 right-0 w-80 md:w-96 transform transition-all duration-700 cubic-bezier(0.16, 1, 0.3, 1) z-[90] ${selectedIso ? 'translate-x-0' : 'translate-x-full'} shadow-[-20px_0_30px_rgba(0,0,0,0.5)]`}>
          <CountryDetails country={countryByIso[selectedIso]} onClose={() => setSelectedIso(null)} />
        </aside>

        {/* 下部パネル: アナリティクス & LIVEニュース - Z-INDEXを最前面(z-[100])に固定 */}
        <footer className={`absolute bottom-0 left-0 right-0 z-[100] bg-slate-950/95 backdrop-blur-3xl border-t border-primary/30 transition-all duration-700 cubic-bezier(0.16, 1, 0.3, 1) flex flex-col ${isAnalyticsOpen ? 'h-[calc(100vh-3.5rem)]' : 'h-10'} shadow-[0_-20px_50px_rgba(0,0,0,0.8)]`}>
          <button onClick={() => setIsAnalyticsOpen(!isAnalyticsOpen)} className="h-10 w-full flex items-center justify-center gap-3 text-[10px] font-bold tracking-[0.4em] text-primary/70 hover:text-primary hover:bg-primary/5 transition-all shrink-0 border-b border-white/5 group">
